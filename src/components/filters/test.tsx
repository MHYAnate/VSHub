...
const AvailableVendors: React.FC<VendorProps> = ({
  ...
  const [ratings, setRatings] = useState<RateValue[]>([]);  // Initialize as empty array
    ...
    return currentPosts.map((vendor) => (
      ...
      >
        ...
        <button
          onClick={() =>
            router.push(
              `/vendorWorkSpace` + "?" + set("docid", `${vendor.docid}`) + "?" + set("id", `${raterDetail?.docid}`) + "?" + set("name", `${raterDetail?.name}`) + "?" + set("src", `${raterDetail?.src}`)
            )
          }

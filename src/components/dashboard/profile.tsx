...
import { fetchRatings, resetCardRating, type RateValue } from '@/lib/store/features/ratingSlice'
  ...
  const tabContent = {
    ...
    services: (
      <div className="space-y-4">
        {[1, 2, 3, 4, 5].map((num) => (
          <div key={num} className="flex justify-between items-center border-b border-gray-200 pb-2">
            <span className="text-gray-700">{profileDetails?.[`service${num}` as keyof ProfileValues]}</span>
            <span className="font-semibold text-gray-900">{profileDetails?.[`s${num}Price` as keyof ProfileValues]}</span>
          </div>
        ))}
      </div>
    ),
        ...
        <div className="bg-white rounded-lg shadow-xl overflow-hidden">
          <div className="md:flex">
            ...
            <div className="p-8">
              ...
              <h1 className="mt-2 text-3xl font-bold">{profileDetails?.name}</h1>

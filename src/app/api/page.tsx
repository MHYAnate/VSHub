  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      <header className="mb-8 text-center">
        <h1 className="text-4xl font-bold text-gray-900 mb-2">Sspot1 API Documentation</h1>
  ...
  availableTo: "string",
        ...
        <Section title="Available Data">
          <div className="grid grid-cols-1">
            <div>
              <h3 className="text-xl font-semibold mb-4">Profiles ({profiles.length})</h3>
              <div className="bg-gray-100 p-4 rounded-lg max-h-fit">
                {profiles.map((profile) => (
                  <div key={profile.docid} className="mb-4 p-4 bg-white rounded shadow">
                    <p className="font-semibold">{profile.name}</p>
                    <p className="text-sm text-gray-600">{profile.email}</p>
                    <p className="text-sm text-gray-500">{profile.service}</p>
                    <div><RateComponent id={profile.docid} profiles={ratings}/></div>
                  </div>
                ))}
              </div>
            </div>
